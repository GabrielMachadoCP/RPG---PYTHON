...
def T21() :
    return """\nO ferimento não teve qualquer efeito sobre a Besta Sangrenta, que continua a atacá-lo tão furiosamente quanto antes. Continue o combate e, logo que vença uma Série de Ataques. Teste sua Sorte. Se você tiver sorte, vá para 97. Se não tiver sorte, vá para 116.\n"""
...
def T56():
    return "\nVocê vê que a obstrução é causada por um objeto grande e marrom, parecendo um rochedo. Você o toca com a mão e fica surpreso ao descobrir que é macio e esponjoso. Se você quiser tentar subir por cima dele, vá para 373. Se preferir cortá-lo ao meio com sua espada, vá para 215.\n"
...
def T114():
    return "\nO Homem da Caverna está usando uma munhequeira de couro com quatro pequenos crânios de rato pendurados. Se você quiser colocá-la no seu próprio pulso, vá para 336. Se preferir prosseguir para o norte, vá para 298.\n"
...
def T117():
    return "\nDepois de uma longa caminhada pelo túnel, você chega a um beco sem saída. Um grande espelho, que vai do chão até o teto, está colocado na parede do fundo. Na penumbra, você só consegue visualizar vagamente o seu próprio reflexo. Se quiser olhar o espelho mais de perto, vá para 329. Se preferir fazer a longa caminhada de volta para a última encruzilhada no túnel, a fim de prosseguir para o leste, vá para 135.\n"
...
def T119():
    return """\nAdiante, você pode ver um grande obstáculo no chão do túnel, embora esteja escuro demais para saber exatamente o que é.
    As pegadas molhadas que você vem seguindo continuam até a obstrução. Se você quiser continuar para o leste, volte para 56. Se preferir voltar para a encruzilhada e seguir para o oeste, vá para 293.\n"""
...
def T121():
    return """\nO Anão olha para os dados. “Você não é muito bom nesse jogo, é?”, graceja. “Lamento, mas você terá que sofrer uma penalidade antes de continuar.”
...
def T122():
    return """\nÀ sua frente, há dois lances de escadas de pedra, separados por um corrimão de crânios de ratazana.
    Você pode subir pelo lance de escadas da esquerda - vá para 176 - ou pelo da direita - vá para 384.\n"""


def T123():
    return """\nO colar é um amuleto de força.
    Some 1 ponto de HABILIDADE e 1 ponto de ENERGIA e continue na sua missão.
    Vá para 282.\n"""


def T124():
    return """\nVocê abre o alçapão e sobe os degraus correndo, chegando a um aposento profundamente iluminado por lanternas.
...
def T125():
    return """\nVocê caminha para a porta na ponta dos pés, enquanto Erva segue tagarelando.
    Teste sua Sorte.
...
def T127():
    return """\nA única maneira possível de sair do salão, tanto quanto você pode ver, é usando um escorrega na parede norte. Você resolve arriscar e sobe no escorrega. Desce deslizando suavemente e aterrissa de costas em outro aposento.
    Volte para 90.\n"""
...
def T130():
    return """\nOs Hobgoblins param de lutar imediatamente. Eles não entendem o que você está dizendo e rosnam agressivamente. Em seguida, desembainham as espadas curtas e avançam para atacá-lo. Lute com um de cada vez.
                        HABILIDADE      ENERGIA
...
def T180():
    return """\nVocê avança na direção da Besta Sangrenta; de repente, sente-se fraco. O gás que emana da poça é altamente tóxico, e você vai ao chão, inconsciente. Teste sua Sorte. Se você tiver sorte, volte para 53. Se não tiver sorte, vá para 272.\n"""
...
def T194():
    return """\nEm uma plataforma de pedra na parede do túnel, você vê dois livros empoeirados encadernados em couro. Throm expressa seu desprezo pela palavra escrita com um grunhido, insistindo para que você deixe os livros de lado e siga adiante com ele. Você:
...
def T209():
    return "\nVocê fica desolado ao descobrir que não apenas todas as suas Provisões restantes estão encharcadas e imprestáveis para comer, mas também que um dos seus tesouros desapareceu. Risque um item da sua Lista de Equipamentos ou uma de suas jóias ou poções. Você guarda cuidadosamente na mochila as posses que lhe restam and parte para o norte outra vez.\n"
...
def T213():
    return """\nO túnel logo se divide em dois. Você ouve um zumbido que vem do ramo da direita. Quer ir até o Oeste ou continuar ao norte?\n"""


def T214():
    return """\nCaminhando em frente, você vê uma linha vermelha pintada no chão do túnel e nota um aviso na parede que diz: “Armas não são permitidas a partir deste ponto.” Ignorar o aviso?\n"""
...
def T216():
    return """\nReconhecendo a cabeça de serpentes da Medusa, você fecha os olhos para evitar o olhar mortal da criatura que o transformaria em pedra. Quer recuar?\n"""
...
def T222():
    return """\nVocê reconhece a fera - é um MANTÉCORA. Levando a sério a advertência do poema, você fica atento para a cauda dele, de cuja ponta sai uma profusão de espinhos afiados, grossos e duros como dardos de ferro. Você tem um escudo?\n"""
...
def T224():
    return """\nParece não haver como continuar para o norte. Você dá meia-volta e retorna pelo túnel, passando pela cadeira de madeira. Logo chega à encruzilhada e vira à direita para seguir para o oeste.\n"""
...
def T226():
    return """\nA carne contém ervas que lhe aumentarão a força. Some 3 pontos ao seu índice de ENERGIA. Quer sair da câmara e continuar para o oeste?\n"""
...
def T230():
    return "\nO túnel começa a se alargar e abre para uma imensa caverna, de onde você pode ouvir o som de muitas vozes agudas. Você se aproxima silenciosamente da entrada e espia. Cerca de 20 minúsculos seres, com narizes e orelhas compridos, correm em círculo em volta de uma grande efígie de ouro.\n\nO que você deseja fazer?\n1. Andar até eles para conversar?\n2. Tentar se esgueirar e passar por eles?\n3. Beber a Poção da Réplica (se você a tiver)\n"
...
def T232():
    return """\nVocê decide verificar suas armas. Se você estiver desarmado, você vai para a função correspondente. Se ainda estiver com suas armas, você também vai para a função correspondente.\n"""
...
def T235():
    return """\nVocê não tem tempo para reagir antes que um dardo se crave na sua coxa. Você perde 2 pontos de ENERGIA. Se ainda estiver vivo, você vai para a função correspondente.\n"""
...
def T241():
    return """\nUma cortina de veludo marrom fecha uma passagem em arco na parede oriental do túnel.\n"""
...
def T248():
    return "\nAs portas abrem para um túnel que segue para o norte. Você fecha as portas atrás de si e parte mais uma vez.\n"
...
def T258():
    return "\nVocê está exausto e se senta para um descanso na cauda da fera morta. Olhando para baixo, a seus pés, você de repente nota um anel de ferro que se destaca na areia. \n"


def T259():
    return "\nIgnorando a dor, você continua correndo. À sua frente, vê um rio subterrâneo que corre de leste para oeste atravessando a caverna, com uma ponte de madeira que liga uma margem a outra. Você olha para trás e vê os Trogloditas no seu encalço. Se você quiser correr pela ponte, vá para 318. Se desejar mergulhar no rio, volte para 47.\n"
...
def T263():
    return "\nA porta abre para um outro túnel. Caminhando para o oeste, você logo chega a uma porta na parede norte. Quer abrir a porta?\n"
